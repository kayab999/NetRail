...
**Open** `POST /api/open`: rate-limit `check_open` → `validate_open_url` (scheme, embedded creds, DDG `uddg` unwrap depth ≤ 5, then host policy) → **DNS pin `pin_open_host` (A15, 2026-08-01)** — resolves the hostname via the system resolver and re-runs the IP blocklist on every answer before spawn; unresolvable hosts fail closed `OPEN_URL_DNS_UNRESOLVABLE` (time-based TTL-flip rebinding remains a documented residual; the browser re-resolves independently) → browser discovery (`desktop-files` dirs + `which`/`flatpak-spawn` fallback, `browsers.rs:97-155`) → spawn with `LD_PRELOAD` stripped and flatpak host wrap → `record_visit`.
...
**Enterprise gaps (beyond §2 findings):** ~~no systemd/launchd units~~ (see `packaging/netrail-api.service`, 2026-08-01) or healthcheck semantics beyond `/api/health` (Docker HEALTHCHECK exists); ~~no backup/restore of the SQLite DB~~ (see `scripts/backup-db.sh`, 2026-08-01); ~~no read-only mode~~ (`NETRAIL_READONLY=1`, 2026-08-01); no multi-user/role model; no egress proxy/TLS pinning config for backends; no metrics (SLO/SLI impossible); ~~no structured logs for ingestion~~ (`NETRAIL_LOG_JSON=1`, 1.6.0); ~~audit log has no rotation/retention policy~~ (1.6.0); ~~no webview E2E~~ (matrix #9, v1.6.1); Windows/macOS unsupported (Linux-only discovery, CI, packaging); ~~no DB schema versioning~~ (A11, 1.5.0).
...
- **Verdict:** strong release discipline for a 1.x solo project; version SSOT across 5 files with a CI gate is exemplary. Gaps closed 2026-08-01: artifact signing (cosign keyless, verified live on v1.6.1), `Swatinem/rust-cache` (the ~11 min build), dependency-audit steps (`cargo audit` pinned 0.22.2 / `npm audit`, both gated in CI). SBOM-in-bundle (E2) closed 2026-08-02: the Rust inventory is embedded in every binary at build time (`netrail-api --sbom`, from `build.rs` reading `Cargo.lock`) and the full `SBOM.txt` is packaged into the `.deb`/`.rpm`/AppImage at `/usr/share/netrail/SBOM.txt` (release CI asserts presence). No Windows/macOS matrix remains out of scope.
