...
**SBOM / supply chain:** every release ships `SBOM.txt` (component inventory from `Cargo.lock` + `requirements.lock`), and it is pinned in the artifacts two ways. The Python image installs from `requirements.lock` (pinned closure generated by `scripts/generate-requirements-lock.sh`; `requirements.txt` keeps the human-edited lower bounds). `.dockerignore` keeps the build context to declared `COPY` sources only:
